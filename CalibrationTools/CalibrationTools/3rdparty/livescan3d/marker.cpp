...
bool MarkerDetector::GetMarker(cv::Mat &img, MarkerInfo &marker)
{
	...
	for (unsigned int i = 0; i < contours.size(); i++)
	{
		...
		{	
			...
			vector<cv::Point2f> cornersFloat2(nMarkerCorners);
			vector<cv::Point3f> points3D;			

			for (int i = 0; i < nMarkerCorners; i++)
			{
				cornersFloat2[i] = cv::Point2f(cornersFloat[i].x, cornersFloat[i].y);
			}
...
void MarkerDetector::GetMarkerPoints(vector<cv::Point3f> &pts)
{
	pts.push_back(cv::Point3f(0.0f, -1.0f, 0.0f));
	pts.push_back(cv::Point3f(-1.0f, -1.6667f, 0.0f));
	pts.push_back(cv::Point3f(-1.0f, 1.0f, 0.0f));
	pts.push_back(cv::Point3f(1.0f, 1.0f, 0.0f));
	pts.push_back(cv::Point3f(1.0f, -1.6667f, 0.0f));
}
