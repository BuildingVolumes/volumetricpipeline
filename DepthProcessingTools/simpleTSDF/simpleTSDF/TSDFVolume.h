...
*/
#define IND2LINEAR(i,j,k, w,h,d) (((k)*(w)*(h)) + ((j)*(h)) + (i))
		...
		for (int k = 0; k < res[2]; k++) {
			for (int j = 0; j < res[1]; j++) {
				for (int i = 0; i < res[0]; i++) {
					...
					grid[ind].sdf = (grid[ind].c - center).norm() - radius;
				}
			}
		}
	}
	...
	*/
	...
	void PolygoniseTri(int ii0, int ii1, int ii2, int ii3, float isolevel, std::vector<TRIANGLE> &triangles) {
		...
		case 5:
			...
			vv0 = VertexInterp(p0, p1, val0, val1);
			vv1 = VertexInterp(p1, p2, val1, val2);
			vv2 = VertexInterp(p2, p3, val2, val3);
		...
		for (int i =1; i < res[0]-1;i++) {
			for (int j = 1; j < res[1]-1; j++) {
				for (int k = 1; k < res[2]-1; k++) {
					...
					indices[1] = IND2LINEAR(i+0, j+1, k+0, res[0], res[1], res[2]);
					indices[2] = IND2LINEAR(i+1, j+1, k+0, res[0], res[1], res[2]);
					indices[3] = IND2LINEAR(i+0, j+0, k+1, res[0], res[1], res[2]);

					indices[4] = IND2LINEAR(i + 0, j + 0, k + 1, res[0], res[1], res[2]);
					indices[5] = IND2LINEAR(i + 0, j + 1, k + 1, res[0], res[1], res[2]);
					indices[6] = IND2LINEAR(i + 1, j + 1, k + 0, res[0], res[1], res[2]);
					indices[7] = IND2LINEAR(i + 0, j + 1, k + 0, res[0], res[1], res[2]);

					indices[8]  = IND2LINEAR(i + 0, j + 0, k + 1, res[0], res[1], res[2]);
					indices[9]  = IND2LINEAR(i + 0, j + 1, k + 1, res[0], res[1], res[2]);
					indices[10] = IND2LINEAR(i + 1, j + 1, k + 1, res[0], res[1], res[2]);
					indices[11] = IND2LINEAR(i + 1, j + 1, k + 0, res[0], res[1], res[2]);

					indices[12] = IND2LINEAR(i + 1, j + 0, k + 0, res[0], res[1], res[2]);
					indices[13] = IND2LINEAR(i + 1, j + 0, k + 1, res[0], res[1], res[2]);
					indices[14] = IND2LINEAR(i + 1, j + 1, k + 0, res[0], res[1], res[2]);
					indices[15] = IND2LINEAR(i + 0, j + 0, k + 1, res[0], res[1], res[2]);

					indices[16] = IND2LINEAR(i + 0, j + 0, k + 0, res[0], res[1], res[2]);
					indices[17] = IND2LINEAR(i + 1, j + 0, k + 0, res[0], res[1], res[2]);
					indices[18] = IND2LINEAR(i + 0, j + 0, k + 1, res[0], res[1], res[2]);
					indices[19] = IND2LINEAR(i + 1, j + 1, k + 0, res[0], res[1], res[2]);

					indices[20] = IND2LINEAR(i + 0, j + 0, k + 1, res[0], res[1], res[2]);
					indices[21] = IND2LINEAR(i + 1, j + 0, k + 1, res[0], res[1], res[2]);
					indices[22] = IND2LINEAR(i + 1, j + 1, k + 1, res[0], res[1], res[2]);
					indices[23] = IND2LINEAR(i + 1, j + 1, k + 0, res[0], res[1], res[2]);

//					PolygoniseTri(indices[0],indices[2],indices[1],indices[3], isolevel, triangles);
					PolygoniseTri(indices[0], indices[1], indices[2], indices[3], isolevel, triangles);
					//PolygoniseTri(indices[0], indices[1], indices[3], indices[2], isolevel, triangles);
		//			PolygoniseTri(indices[0], indices[2], indices[1], indices[3], isolevel, triangles);
	//				PolygoniseTri(indices[0], indices[2], indices[3], indices[1], isolevel, triangles);
//					PolygoniseTri(indices[1], indices[0], indices[2], indices[3], isolevel, triangles);
	...
	//int PolygoniseCell(double isolevel, std::vector<TRIANGLE> &triangles, int xi, int yi, int zi) {
	//	int i, ntriang;
	//	int cubeindex;
 ...
 //  */
	...
	//	Eigen::Vector3d p[8];
	//	float v[8];
	//	v[0] = grid[IND2LINEAR(xi-1, yi-1, zi-1, res[0], res[1], res[2])].sdf;
	//	v[1] = grid[IND2LINEAR(xi  , yi-1, zi-1, res[0], res[1], res[2])].sdf;
	//	v[2] = grid[IND2LINEAR(xi  , yi-1, zi  , res[0], res[1], res[2])].sdf;
	//	v[3] = grid[IND2LINEAR(xi-1, yi-1, zi  , res[0], res[1], res[2])].sdf;
	//	v[4] = grid[IND2LINEAR(xi-1, yi  , zi-1, res[0], res[1], res[2])].sdf;
	//	v[5] = grid[IND2LINEAR(xi  , yi  , zi-1, res[0], res[1], res[2])].sdf;
	//	v[6] = grid[IND2LINEAR(xi-1, yi  , zi  , res[0], res[1], res[2])].sdf;
	//	v[7] = grid[IND2LINEAR(xi  , yi  , zi  , res[0], res[1], res[2])].sdf;

	//	p[0] = grid[IND2LINEAR(xi - 1, yi - 1, zi - 1, res[0], res[1], res[2])].c;
	//	p[1] = grid[IND2LINEAR(xi    , yi - 1, zi - 1, res[0], res[1], res[2])].c;
	//	p[2] = grid[IND2LINEAR(xi    , yi - 1, zi    , res[0], res[1], res[2])].c;
	//	p[3] = grid[IND2LINEAR(xi - 1, yi - 1, zi    , res[0], res[1], res[2])].c;
	//	p[4] = grid[IND2LINEAR(xi - 1, yi    , zi - 1, res[0], res[1], res[2])].c;
	//	p[5] = grid[IND2LINEAR(xi    , yi    , zi - 1, res[0], res[1], res[2])].c;
	//	p[6] = grid[IND2LINEAR(xi - 1, yi    , zi    , res[0], res[1], res[2])].c;
	//	p[7] = grid[IND2LINEAR(xi    , yi    , zi    , res[0], res[1], res[2])].c;
	...
	//		vertlist[1] = VertexInterp(isolevel, p[1], p[2], v[1], v[2]);
	//	if (edgeTable[cubeindex] & 4)
	//		vertlist[2] = VertexInterp(isolevel, p[2], p[3], v[2], v[3]);
	//	if (edgeTable[cubeindex] & 8)
	//		vertlist[3] = VertexInterp(isolevel, p[3], p[0], v[3], v[0]);
	//	if (edgeTable[cubeindex] & 16)
	//		vertlist[4] = VertexInterp(isolevel, p[4], p[5], v[4], v[5]);
	//	if (edgeTable[cubeindex] & 32)
	//		vertlist[5] = VertexInterp(isolevel, p[5], p[6], v[5], v[6]);
	//	if (edgeTable[cubeindex] & 64)
	//		vertlist[6] = VertexInterp(isolevel, p[6], p[7], v[6], v[7]);
	//	if (edgeTable[cubeindex] & 128)
	//		vertlist[7] = VertexInterp(isolevel, p[7], p[4], v[7], v[4]);
	//	if (edgeTable[cubeindex] & 256)
	//		vertlist[8] = VertexInterp(isolevel, p[0], p[4], v[0], v[4]);
	//	if (edgeTable[cubeindex] & 512)
	//		vertlist[9] = VertexInterp(isolevel, p[1], p[5], v[1], v[5]);
	//	if (edgeTable[cubeindex] & 1024)
	//		vertlist[10] = VertexInterp(isolevel, p[2], p[6],v[2], v[6]);
	//	if (edgeTable[cubeindex] & 2048)
	//		vertlist[11] = VertexInterp(isolevel, p[3], p[7], v[3], v[7]);
	//	/* Create the triangle */
	//	ntriang = 0;
